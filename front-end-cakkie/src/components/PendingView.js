...
import '../css/StatusView.css'; // Specific CSS for PendingView
...
function PendingView() {
  ...
  // Hàm tính tổng số comment dựa trên quantity của pendingComments
  const calculateTotalComments = (pendingComments) => {
    return pendingComments.reduce((total, comment) => total + comment.quantity, 0);
  };
  ...
  const handleProductClick = (productName) => {
    // Navigate to the ProductDetailPendingView with category and product name
    navigate(`/pending-management/${category}/${encodeURIComponent(productName)}`);
  };

  return (
    <div className="approve-view">
      <button className="back-button" onClick={() => navigate('/review-management')}>Back</button>
      <h2>Pending View ({category.charAt(0).toUpperCase() + category.slice(1)})</h2>
      <input
        type="text"
        placeholder="Search..."
        ...
        <div className="approve-product-grid">
          {displayProducts().map((prod, index) => (
            ...
            >
              ...
              <h3>{prod.name}</h3>
              <p>{calculateTotalComments(prod.pendingComments)} comments</p> {/* Tính tổng số comment */}
            </div>
          ))}
        </div>
      ) : (
